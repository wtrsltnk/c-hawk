...
echo.
echo @echo Building %BIN_DIR%\%PROJECT_NAME%.exe with %TCC%>%PROJECT_ROOT%\build\temp_build.bat
...
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat

@rem Compile all sources to objects
for /f "tokens=*" %%a in (%PROJECT_ROOT%\project-files.txt) do (
    echo @echo Compiling %%~a>>%PROJECT_ROOT%\build\temp_build.bat
    echo @%TCC% -c %INCLUDES% -o %OBJ_DIR%\%%~na.o %PROJECT_ROOT%\%%~a>>%PROJECT_ROOT%\build\temp_build.bat
    echo @if ERRORLEVEL 1 goto :errorHandling>>%PROJECT_ROOT%\build\temp_build.bat
)

@rem Link all objects to one exe
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo Linking all objects into %BIN_DIR%\%PROJECT_NAME%.exe>>%PROJECT_ROOT%\build\temp_build.bat
echo|set /p=@%TCC% -o %BIN_DIR%\%PROJECT_NAME%.exe>>%PROJECT_ROOT%\build\temp_build.bat
for /f "tokens=*" %%a in (%PROJECT_ROOT%\project-files.txt) do (
    echo | set /p dummy=%OBJ_DIR%\%%~na.o>>%PROJECT_ROOT%\build\temp_build.bat
)

echo.>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat
...
if exist %PROJECT_ROOT%\project-binaries.txt (
    for /f "tokens=*" %%a in (%PROJECT_ROOT%\project-binaries.txt) do (
        echo copy %%~a %BIN_DIR%>>%PROJECT_ROOT%\build\temp_build.bat
    )
)

echo @echo Done building %PROJECT_NAME%>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat
echo @goto :exit>>%PROJECT_ROOT%\build\temp_build.bat
echo :errorHandling>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo Error compiling %PROJECT_NAME%>>%PROJECT_ROOT%\build\temp_build.bat
echo @echo.>>%PROJECT_ROOT%\build\temp_build.bat
echo :exit>>%PROJECT_ROOT%\build\temp_build.bat

@echo on
@call %PROJECT_ROOT%\build\temp_build.bat
@echo off
@del %PROJECT_ROOT%\build\temp_build.bat
